var fs = require("fs");
var path = require("path");
var FtpDeploy = require("ftp-deploy");
var ftpDeploy = new FtpDeploy();
let child_process = require('child_process');
const { protocol } = require("socket.io-client");

var argv = require("minimist")(process.argv.slice(2));

ftpDeploy.on("uploading", function (data) {
    data.totalFilesCount; // total file count being transferred
    data.transferredFileCount; // number of files transferred
    data.filename; // partial path with filename being uploaded
});
ftpDeploy.on("uploaded", function (data) {
    console.log(data); // same data as uploading event
});
ftpDeploy.on("log", function (data) {
    console.clear();
    data.transferredFileCount = 0;
    console.log(data); // same data as uploading event
});

async function exec(cmd) {
    console.log(cmd);
    return child_process.execSync(cmd);
}
async function setRevision() {
    let revision = child_process.execSync('git rev-parse HEAD').toString().trim();
    fs.writeFileSync(path.join(__dirname, '../www', 'revision'), JSON.stringify({ revision: revision }), 'utf8');
    return;
}

let cfg_base = {
    sftp: true, // NOTE that this was username in 1.x
    user: "root", // NOTE that this was username in 1.x
    password: "1@Lk.vps", // optional, prompted if none given
    host: "129.121.53.56",
    port: 22022,
    include: ["*", "**/*"], // this would upload everything except dot files
    forcePasv: true, // Passive mode is forced (EPSV command is not sent)
    localRoot: path.join(__dirname, "../www"),
};

let cfgs = {
    'hom': {
        default: {},
        segin_web: {
            remoteRoot: "/var/www/html/seginhom/web/",
            exclude: ["assets/**/*", "svg/**/*"],
            build: `ng build --configuration hom --project=web --base-href /web/`
        },
        segin_app: {
            remoteRoot: "/var/www/html/seginhom/app",
            exclude: ["assets/**/*", "svg/**/*"],
            build: `ng build --configuration hom --project=app --base-href /app/`
        },
    },
    'prod': {
        default: {},
        assets: {
            localRoot: path.join(__dirname, "../src/assets"),
            remoteRoot: "/var/www/html/assets/segin/",
        },
        segin_site: {
            localRoot: path.join(__dirname, "../src/app/site"),
            remoteRoot: "/var/www/html/segin/",
            exclude: ["assets/**/*", "svg/**/*"],
        },
        segin_web: {
            remoteRoot: "/var/www/html/segin/web/",
            exclude: ["assets/**/*", "svg/**/*"],
            build: `ng build --configuration production --project=web --base-href /web/`
        },
        segin_app: {
            remoteRoot: "/var/www/html/segin/app",
            exclude: ["assets/**/*", "svg/**/*"],
            build: `ng build --configuration production --project=app --base-href /app/`
        },
    }
};

(async () => {
    if (!argv) throw new Error('Nenhum argumento passado para deploy');
    if (!argv.app) throw new Error('Nenhum app definido para deploy');
    if (!argv.env) throw new Error('Ambiente não definido para o deploy');

    let cfg_deploy = Object.assign({}, cfg_base, cfgs[argv.env].default, cfgs[argv.env][argv.app]);
    console.clear();
    if (cfg_deploy.build)
        await exec(cfg_deploy.build);

    await setRevision();

    // ASSETS
    if (argv.assets)
        await ftpDeploy.deploy(Object.assign({}, cfg_deploy, {
            include: ["assets/**/*"],
            exclude: [],
            remoteRoot: "/var/www/html"
        }))

    // CODIGO APLICAÇÃO
    return Promise.resolve(true)
        .then(async start => {
            console.log('start', cfg_deploy);
            
            return ftpDeploy.deploy(cfg_deploy);
        })
        .then(res => console.log("finished"))
        .catch(err => console.log(err));
})();